...
def process_match_prediction(
    ...
    # Target Price Logic (Value Betting)
    # ----------------------------------
    # Odd minima para garantir ROI de 5% sobre a probabilidade calibrada
    target_odd = StatisticalAnalyzer.calculate_target_price(confidence, min_roi=0.05) if confidence > 0 else 0.0
    ...
    # Value Betting Update (30/01/2026)
    # Exibe PREÇO ALVO para educar usuário sobre valor
    if target_odd < 999.0:
        feedback_text += f"\n💰 Target Price: @{target_odd:.2f} (Valuation)"
    else:
    ...
    try:
        ...
        if not home_games.empty and not away_games.empty:
            ...
            # --- CYBORG UPGRADE: INJECT NEURAL PARAMS ---
            if 'neural_params' in match_data:
                 advanced_metrics['neural_params'] = match_data['neural_params']
            ...
            # --- NEURAL UPGRADE (MAIN DRIVER) ---
            if neural_engine and neural_engine.is_trained:
                try:
                    # Re-score Top Picks using Neural Network
                    # We convert statistical picks to Neural Candidates
                    neural_picks = neural_engine.predict_match(match_data, top_picks, df_history)
                    
                    # Create a mapping to update original picks
                    neural_map = {p['prediction_label']: p for p in neural_picks}
                    
                    updated_picks = []
                    for pick in top_picks:
                        label = pick['Seleção']
                        if label in neural_map:
                            n_pred = neural_map[label]
                            # OVERWRITE Statistical Probability with Neural Confidence
                            pick['Prob'] = n_pred['confidence']
                            pick['Source'] = 'Neural_Network'
                            # Should we update Odd? No, Odd is market data.
                        updated_picks.append(pick)
                    
                    top_picks = updated_picks
                    
                    # Also re-score Suggestions if needed (Optional, focusing on Top 7 for now)
                    
                except Exception as e:
                    print(f"⚠️ Neural Upgrade Error: {e}")
            ...
            # Salva Top 7 (Agora Neural Validated se disponível)
            for pick in top_picks[:7]:
                # Usa dado estruturado confiável (sem regex)
                line_val = pick.get('raw_line', 0.0) 
                # Save as Statistical? Maybe change model_version to distinguish?
                # User asked: "Salvar no banco igual já fazemos". So keep 'Statistical' or 'Top7' category.
                # But value is now Neural.
