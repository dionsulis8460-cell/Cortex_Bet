...
export async function GET(request: Request) {
  ...
  const microserviceUrl = `http://localhost:8000/api/predictions?date=${date}&league=${league}&status=${status}&top7_only=${top7Only}&sort_by=${sortBy}`;
  ...
  try {
    ...
    return NextResponse.json({
      success: false,
      error: error.message || "Failed to connect to Prediction Engine. Is the server running?",
      matches: []
    }, { status: 500 });
  }
}
