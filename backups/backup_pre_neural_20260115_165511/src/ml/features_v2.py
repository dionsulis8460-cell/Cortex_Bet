...
def create_advanced_features(df: pd.DataFrame, window_short: int = 3, window_long: int = 5) -> tuple:
    ...
    # Home Stats
    # Home Stats
    # Colunas: match_id, start_ts, tourn_id, team_id, opp_id, corners, shots, goals, goals_conceded, corners_ht, corners_conceded_cantos, da, blocked, crosses, tackles, interc, clearances, recov, momentum, momentum_conceded, is_home
    df_home = df[['match_id', 'start_timestamp', 'tournament_id', 'home_team_id', 'away_team_id', 'corners_home_ft', 'shots_ot_home_ft', 'home_score', 'away_score', 'corners_home_ht', 'corners_away_ft', 'dangerous_attacks_home', 'blocked_shots_home', 'crosses_home', 'tackles_home', 'interceptions_home', 'clearances_home', 'recoveries_home', 'momentum_home', 'momentum_away']].copy()
    df_home.columns = ['match_id', 'start_timestamp', 'tournament_id', 'team_id', 'opponent_id', 'corners', 'shots', 'goals', 'goals_conceded', 'corners_ht', 'corners_conceded', 'dangerous_attacks', 'blocked_shots', 'crosses', 'tackles', 'interceptions', 'clearances', 'recoveries', 'momentum', 'momentum_conceded']
    df_home['is_home'] = 1
    
    # Away Stats
    # Away Stats
    df_away = df[['match_id', 'start_timestamp', 'tournament_id', 'away_team_id', 'home_team_id', 'corners_away_ft', 'shots_ot_away_ft', 'away_score', 'home_score', 'corners_away_ht', 'corners_home_ft', 'dangerous_attacks_away', 'blocked_shots_away', 'crosses_away', 'tackles_away', 'interceptions_away', 'clearances_away', 'recoveries_away', 'momentum_away', 'momentum_home']].copy()
    df_away.columns = ['match_id', 'start_timestamp', 'tournament_id', 'team_id', 'opponent_id', 'corners', 'shots', 'goals', 'goals_conceded', 'corners_ht', 'corners_conceded', 'dangerous_attacks', 'blocked_shots', 'crosses', 'tackles', 'interceptions', 'clearances', 'recoveries', 'momentum', 'momentum_conceded']
    df_away['is_home'] = 0
    ...
    feature_cols = ['corners', 'shots', 'goals', 'corners_conceded', 'dangerous_attacks', 'corners_ht', 'blocked_shots', 'crosses', 'tackles', 'interceptions', 'clearances', 'recoveries', 'momentum', 'momentum_conceded']
    ...
    # Fallback simples para compatibilidade: usa 'goals' que já existe
    ...
    # --- Derived Momentum Features (PhD Scope) ---
    # momentum_diff: Quanto o time pressiona mais que o adversário (Net Pressure)
    # momentum_per_min: Intensidade (Volume / 90 min) -- Aproximado
    # Importante: Usamos as médias móveis (EMA 5g) para projeção
    
    if 'ema_momentum_5g' in team_stats.columns and 'ema_momentum_conceded_5g' in team_stats.columns:
        team_stats['momentum_diff_5g'] = team_stats['ema_momentum_5g'] - team_stats['ema_momentum_conceded_5g']
        team_stats['momentum_ratio_5g'] = team_stats['ema_momentum_5g'] / (team_stats['ema_momentum_conceded_5g'] + 1.0) # Avoid div/0
    ...
    core_metrics = [
        ...
        'blocked_shots', 'crosses', 'momentum', 'momentum_conceded'
    ]
    ...
    display_metrics_cols = [
        ...
        'home_momentum_diff_5g', # Added based on features logic
        'home_avg_momentum_general', 'away_avg_momentum_general' # Fallback if needed
    ]
    ...
    # =========================================================================
    # Removing features that failed ADF Test (p-value >= 0.05)
    # These features exhibit Random Walk behavior and cause OOT Overfitting.
    NON_STATIONARY_FEATURES = [
        ...
        # Momentum (Raw values drift, diff is safe)
        'home_ema_momentum_10g', 'home_ema_momentum_5g', 
        'home_std_momentum_5g', 'home_ema_momentum_3g', 
        'home_std_momentum_10g',
        'away_std_momentum_conceded_10g', 'away_std_momentum_conceded_5g', 
        'away_ema_momentum_conceded_10g', 'away_ema_momentum_conceded_5g', 
        'away_ema_momentum_conceded_3g',

        # Structural/Const (Drift or Zero Variance)
        'home_league_pos', 'away_league_pos',
        'away_entropy_corners', 'home_entropy_corners'
    ]
...
def prepare_features_for_prediction(home_id, away_id, db_manager, window_long=5):
    ...
    Returns:
        pd.DataFrame: Features prontas para predição (single row DataFrame)
    """
    df = db_manager.get_historical_data()
    
    # Filtra jogos relevantes com robustez (Features V2.1 Academic Fix)
    # Usa .loc e .copy() para garantir alinhamento e evitar SettingWithCopyWarning
    relevant_games = df.loc[
        (df['home_team_id'] == home_id) | (df['away_team_id'] == home_id) |
        (df['home_team_id'] == away_id) | (df['away_team_id'] == away_id)
    ].copy()
    
    # --- ROBUST FEATURE ENGINEERING (Sprint 8) ---
    # Problema: Jogos atípicos (ex: 1 escanteio) criam viés de recência na EMA.
    # Solução: Winsorization (Clipping). Floor de 3.0 escanteios.
    # Justificativa: Menos de 3 escanteios é ruído estatístico (ver docs/ml_investigation_results.md)
    relevant_games['corners_home_ft'] = relevant_games['corners_home_ft'].clip(lower=3.0)
    relevant_games['corners_away_ft'] = relevant_games['corners_away_ft'].clip(lower=3.0)
    ...
    # Gera features V3
    # CORREÇÃO: Unpacking robusto da nova assinatura (Audit V3)
    X, y, timestamps, df_display = create_advanced_features(df_combined, window_long=window_long)
    
    # Pegamos a última linha do X (features limpas) - já é um DataFrame
    features_df = X.iloc[[-1]].copy()
    
    # Recupera display metrics da última linha e faz merge
    display_row = df_display.iloc[[-1]].copy()
    
    # Merge eficiente: adiciona colunas de display ao features_df
    for col in display_row.columns:
        features_df[col] = display_row[col].values[0] # .values[0] para pegar o escalar


    # Verificação de Histórico
    # Usamos janela de 5 jogos como base de presença de dados
    col_check = 'home_avg_corners_5g' if 'home_avg_corners_5g' in features_df else (
                'home_avg_corners_general' if 'home_avg_corners_general' in features_df else None)
    
    if col_check and features_df[col_check].iloc[0] == 0 and len(relevant_games) < 5:
    ...
    # GARANTIA: Retorna DataFrame com features do modelo + display metrics
    return features_df
