...
def analyze_match_url() -> None:
    ...
    try:
        ...
        # 3. Preparar Dependências do Motor
        df_history = db.get_historical_data()
        
        predictor = ProfessionalPredictor()
        if not predictor.load_model():
            print(f"{Colors.RED}⚠️ Modelo Profissional não encontrado. Treine-o primeiro (Opção 4).{Colors.RESET}")
            return

        result = process_match_prediction(
        ...
        if 'error' in result:
             print(f"{Colors.RED}❌ Erro na análise: {result['error']}{Colors.RESET}")
        else:
             print("\n" + "="*60)
             ...
             pred_val = result.get('ml_prediction', 0)
             print(f"\n✅ {pred_val} esc | {conf_val}% conf | 💾 Salvo!")
...
def scan_opportunities() -> None:
    ...
    try:
        results = scan_opportunities_core(date_str=target_date, db=db, predictor=predictor, verbose=True)
        if results:
            print("\n" + "=" * 70)
