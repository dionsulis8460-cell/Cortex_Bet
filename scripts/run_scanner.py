...
async def main():
    ...
    # [REFAC] Use Manager AI (Auto-loads models)
    try:
        manager = ManagerAI(db)
    except Exception as e:
        print(f"{Colors.YELLOW}[WARNING] ManagerAI failed to load. Predictions might fail: {e}{Colors.RESET}")
        manager = None
    ...
    try:
        results = await asyncio.to_thread(
            ...
            db=db, 
            manager=manager, # Pass Manager AI
            verbose=True
        )
