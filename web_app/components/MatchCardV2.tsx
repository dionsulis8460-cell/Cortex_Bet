    ...
    <div className="bg-slate-800 rounded-xl border border-slate-700/50 overflow-hidden hover:border-blue-500/30 transition-all">
      ...
      <div className={`p-4 md:p-6 ${match.status === 'inprogress' ? 'bg-gradient-to-r from-slate-800 via-slate-800 to-red-900/10' : match.status === 'finished' ? 'opacity-75' : ''}`}>
        ...
        <div className={`bg-slate-900/50 rounded-lg p-4 mb-4 border ${
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-sm relative w-full">
            {/* Result Badge */}
          ...
          {/* Neural Validator Badge */}
          {match.mainBet.neuralValidator && (
            <div className="mt-3 pt-3 border-t border-slate-700/50 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 bg-slate-800/30 p-2 rounded">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-blue-400 uppercase tracking-widest flex items-center gap-1">
                    🤖 Neural Fair Price
                  </span>
                  <span className="text-[10px] bg-blue-900/30 text-blue-300 px-1.5 py-0.5 rounded border border-blue-500/30">
                    BETA
                  </span>
                </div>
                <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
                  <span className="text-xs text-slate-400">
                    Exp: <span className="text-white font-mono">{match.mainBet.neuralValidator.expectedCorners.toFixed(1)}</span>
                  </span>
                  <div className="flex items-center gap-2">
                      <span className="text-xs text-slate-300">Target Odd:</span>
                      <span className={`text-sm font-bold font-mono px-2 py-0.5 rounded ${
                        match.mainBet.neuralValidator.fairOdd < 2.0 ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-slate-700 text-slate-300'
                      }`}>
                        @{match.mainBet.neuralValidator.fairOdd.toFixed(2)}
                      </span>
                  </div>
                </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              <h4 className="text-white font-bold mb-3">
                ...
                <div className="bg-slate-800/50 rounded-lg overflow-hidden border border-slate-700/50 w-full">
